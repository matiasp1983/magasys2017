...
namespace PL.AdminDashboard
{
    public partial class DetalleProductoIngresosEditar : System.Web.UI.Page
    {
        ...
        private void CargarDetalleProductoIngreso()
        {
            ...
            {
                var lstDetalleProductoIngreso = new BLL.ProductoIngresoBLL().ObtenerDetalleProductoIngreso(Convert.ToInt32(Session[Enums.Session.IdIngresoProductos.ToString()]));
                ...
                if (lstDetalleProductoIngreso.Count > 0)
                {
                    ...
                    foreach (var loDetalleProductoIngreso in lstDetalleProductoIngreso)
                    {
                        ...
                        using (var loRepProductoEdicion = new Repository<ProductoEdicion>())
                            oProductoEdicion = loRepProductoEdicion.Find(p => p.ID_PRODUCTO_EDICION == loDetalleProductoIngreso.COD_PRODUCTO_EDICION);

                        using (var loRepProducto = new Repository<BLL.DAL.Producto>())
                            loNombre = loRepProducto.Find(p => p.ID_PRODUCTO == oProductoEdicion.COD_PRODUCTO).NOMBRE;

                        var loTipoProducto = new TipoProductoBLL().ObtenerTipoProducto(oProductoEdicion.COD_TIPO_PRODUCTO);
                        ...
                        switch (oProductoEdicion.COD_TIPO_PRODUCTO)
                        {
                            case 1:
                                ...
                                oDiarioEdicion = new DiarioEdicion
                                {
                                    ...
                                    NOMBRE = loNombre,
                                    TIPO_PRODUCTO = loTipoProducto.DESCRIPCION,
